// 发酵美食专栏 B 站资源批量调研脚本（复用 bili-search.mjs 的 wbi 签名逻辑）
// 用法：node scripts/coldstart/fermented-research.mjs
// 输出：scripts/coldstart/out/fermented-research.json（含所有关键词的宽口径候选）
...
// 关键词按文章章节组织：东北酸菜 / 四川泡菜 / 贵州酸汤 / 酸菜鱼 / 榨菜
const KEYWORDS = [
  '东北酸菜',
  '东北腌酸菜',
  '酸菜白肉',
...
for (const kw of KEYWORDS) {
  ...
  // 宽口径：≥3 分钟、播放 ≥8000、2020 年后（美食视频普遍比徒步 vlog 短，门槛放宽）
  const kept = items.filter(
    (r) => r.durationSec >= 180 && r.play >= 8000 && r.pubdateISO >= '2020-01-01'
  )
...
fs.writeFileSync(
  path.join(outDir, 'fermented-research.json'),
  JSON.stringify({ generatedAt: new Date().toISOString(), report }, null, 2),
  'utf-8'
)
console.log(`\n已保存: ${outDir}/fermented-research.json`)
