...
function sendInquiry() {
  const subject = encodeURIComponent(`[ChinaTravelHub] ${props.service.name}`)
...
<template>
  <!-- 服务商卡片 -->
  <div class="card-dark p-5 card-hover">
    <div class="flex gap-4">
      <!-- 头像徽章（首字母圆形） -->
      <div class="w-[72px] h-[72px] rounded-xl bg-gradient-to-br from-brand/40 to-slate-700 flex items-center justify-center text-2xl font-bold text-brand-light shrink-0">
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="flex items-center gap-2 flex-wrap mb-1">
          <h3 class="text-[17px] font-semibold text-slate-50 leading-snug">
            {{ service.name }}
          </h3>
          <span
            v-if="service.isVerified"
            class="inline-flex items-center gap-1 bg-emerald-500/15 text-emerald-400 text-[11px] font-medium px-2 py-0.5 rounded-full border border-emerald-500/30"
          >
            ✓ {{ t('services.verified') }}
          </span>
        </div>
        ...
        <div class="flex items-center gap-2 text-[13px] mb-2">
          ...
          <span class="text-slate-300">{{ service.rating.toFixed(1) }}</span>
          <span class="text-slate-500">({{ t('services.reviews', { n: service.reviewCount }) }})</span>
          <span class="chip">{{ t(`services.types.${service.type}`) }}</span>
        </div>

        <!-- 简介 -->
        <p class="text-sm text-slate-400 leading-relaxed mb-2 line-clamp-2">
        ...
        <div class="flex items-center gap-4 text-xs text-slate-500">
          ...
          <span class="text-slate-400">{{ service.priceRange[locale] }}</span>
        </div>
      </div>
    ...
    <!-- 展开详情 -->
    <div v-if="expanded" class="mt-5 pt-5 border-t border-slate-700">
    ...
    <!-- 咨询表单 -->
    <div v-if="showInquiry" class="mt-5 pt-5 border-t border-slate-700">
      <h4 class="text-sm font-semibold text-slate-200 mb-3">
        {{ t('services.sendInquiry') }}
      </h4>
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
        <input v-model="form.name" type="text" :placeholder="t('services.formName')" class="input-dark px-3 py-2 text-sm">
        <input v-model="form.email" type="email" :placeholder="t('services.formEmail')" class="input-dark px-3 py-2 text-sm">
        <input v-model="form.dates" type="text" :placeholder="t('services.formDates')" class="input-dark px-3 py-2 text-sm sm:col-span-2">
        <textarea v-model="form.message" rows="3" :placeholder="t('services.formMessage')" class="input-dark px-3 py-2 text-sm sm:col-span-2 resize-none" />
      </div>
